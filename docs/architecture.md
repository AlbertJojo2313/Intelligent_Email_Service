...
2. Wraps network operations in a robust domain exception handling layer (`exceptions.py`).
3. Queries historical email messages matching client identifiers (From/To/Cc).
4. Groups correspondence by topic/subject (`_normalize_subject`) and resolves multi-message email threads.
5. Preprocesses and compresses email message bodies while handling attachments.
6. Emits structured JSON payloads tailored for LLM prompt context injection.
...
flowchart TD
    subgraph Core Package [intelligent_email_service]
        ...
        MGP -->|pattern matching error mapping| ExLayer[exceptions.py: Domain Exceptions]
        MSGP -.-> ExLayer
        ...
        subgraph Preprocessing [intelligent_email_service.preprocessing]
            Prep --> Cleaner[cleaner.py: HTML / Quote Stripper (Outline)]
            Prep --> Compressor[compressor.py: Context Compressor (Outline)]
        end
...
3. **Mailbox Search & Retrieval**: Query the advisor's mailbox for messages where `ClientID` appears in sender (`from`), recipient (`toRecipients`), or CC fields. Errors are mapped into domain exceptions (`ProviderAuthenticationError`, `ProviderRateLimitError`, `ProviderNotFoundError`).
4. **Subject & Thread Grouping**:
   - Group matched messages by normalized subject line (`_normalize_subject`) and conversation identifier (`conversation_id`).
...
- **Exception Layer**: Custom domain exceptions (`EmailServiceError`, `EmailProviderError`, `EmailRetrievalError`) providing unified error reporting.
- **Provider Layer**:
...
- Uses `NvidiaClient` (NVIDIA AI Cloud / LLM using `deepseek-ai/deepseek-v4-flash`) with local `FallbackGenerator` templates.
